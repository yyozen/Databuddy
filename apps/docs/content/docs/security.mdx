...
Configure allowed origins and IP addresses to restrict who can send analytics data to your website.

#### Allowed Origins

Control which domains can send analytics events to your website. This provides an additional layer of security beyond domain verification.
...
1. When `allowedOrigins` is configured, only requests from those origins are accepted
2. If not configured, the system falls back to standard domain validation
...
<Callout type="info">
  **Backward Compatibility**: Websites without security settings configured will continue to work with standard domain validation. These settings are optional and provide additional security layers.
</Callout>
